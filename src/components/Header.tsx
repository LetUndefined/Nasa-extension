...
const Header = () => {
  const { loading } = useContext(AstronautContext) ?? { data: null, loading: false };
  return (
    <>
      <div className="border-b border-[var(--border-bright)] py-2" style={{ fontFamily: "var(--font-display)" }}>
        <div className="mx-4  flex justify-between items-center ">
          ...
          <h4 className="text-[var(--green)] uppercase text-[10px]  tracking-[0.1rem] font-bold ">{loading ? "Loading" : "Live"}</h4>
        </div>
      </div>
    </>
  );
